...
* This also, technically, gives opertions on any index `0 < k < n` a complexity of $\Omega\left(\min\left\lbrace k, n-k \right\rbrace\right)$, because we could always choose to go from the `tail` when it's closer than the `head`.
